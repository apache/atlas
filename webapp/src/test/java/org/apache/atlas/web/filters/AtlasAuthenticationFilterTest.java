...
public class AtlasAuthenticationFilterTest {
    ...
    private static final String DOAS_PARAM = "doAs";
...
//        doNothing().when(filter).doKerberosAuth(any(), any(), any());

        filter.doFilter(mockRequest, mockResponse, mockFilterChain);

//        verify(filter).doKerberosAuth(mockRequest, mockResponse, mockFilterChain);
    }
...
/*    @Test
    public void testDoKerberosAuthProxyUser() throws Exception {
        ...
        try (MockedStatic<Servlets> mockedServlets = mockStatic(Servlets.class);
            ...
            // Set up the token validation to work properly
            when(mockAuthToken.getUserName()).thenReturn("proxy");
            when(mockAuthToken.toString()).thenReturn("u=proxy&t=kerberos&e=" + (System.currentTimeMillis() + 10000) + "&s=signature&p=proxy");
