...
public class SecureEmbeddedServerTestBase {
    ...
    public void testNoConfiguredCredentialProvider() throws Exception {
        ...
        try {
            ...
            secureEmbeddedServer = new SecureEmbeddedServer(EmbeddedServer.ATLAS_DEFAULT_BIND_ADDRESS, securePort, TestUtils.getWarPath());

            secureEmbeddedServer.server.start();

            fail("Should have thrown an exception");
        } catch (IOException e) {
            assertEquals(e.getMessage(), "No credential provider path cert.stores.credential.provider.path configured for storage of certificate store passwords");
        } finally {
    ...
    public void testMissingEntriesInCredentialProvider() throws Exception {
        ...
        try {
            ...
            fail("No entries should generate an exception");
        } catch (IOException e) {
            assertTrue(e.getMessage().startsWith("No credential entry found for"));
        } finally {
            secureEmbeddedServer.server.stop();
        }
    }
    ...
    public void runOtherSuitesAgainstSecureServer() throws Exception {
        ...
        try {
            ...
            testng.setTestClasses(new Class[] {AdminJerseyResourceIT.class, EntityJerseyResourceIT.class, TypesJerseyResourceIT.class});
