...
public class EntityNotificationListenerV2 implements EntityChangeListenerV2 {
    ...
    private void notifyEntityEvents(List<AtlasEntity> entities, OperationType operationType) throws AtlasBaseException {
        ...
        for (AtlasEntity entity : entities) {
            ...
            messages.add(new KeyValue<>(
                             entity.getGuid(),
                             new EntityNotificationV2(toNotificationHeader(entity), operationType, RequestContext.get().getRequestTime())));
    ...
    private void notifyRelationshipEvents(List<AtlasRelationship> relationships, OperationType operationType) throws AtlasBaseException {
        ...
        for (AtlasRelationship relationship : relationships) {
            ...
            messages.add(new KeyValue<>(
                    ...
                    new EntityNotificationV2(toNotificationHeader(relationship), operationType, RequestContext.get().getRequestTime())));
        }
    ...
    private void sendNotifications(OperationType operationType, List<KeyValue<String, EntityNotificationV2>> messages) throws AtlasBaseException {
        if (!messages.isEmpty()) {
            try {
