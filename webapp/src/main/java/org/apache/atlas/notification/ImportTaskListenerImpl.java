...
public class ImportTaskListenerImpl implements Service, ActiveStateChangeHandler, ImportTaskListener {
    ...
    private static final String ASYNC_IMPORT_TOPIC_PREFIX = "ATLAS_IMPORT_";
    ...
    private void startImportConsumer(AtlasAsyncImportRequest importRequest) {
        try {
            ...
            asyncImportService.updateImportRequest(importRequest);
            if (ObjectUtils.equals(importRequest.getStatus(), AtlasAsyncImportRequest.ImportStatus.FAILED)) {
    ...
    AtlasAsyncImportRequest getNextImportFromQueue() {
        ...
        while (retryCount < maxRetries) {
            try {
                ...
                if (importId == null) {
                    retryCount++;
                    LOG.warn("Still waiting for import request... (Attempt {}/{})", retryCount, maxRetries);
                    continue;
                }
    ...
    boolean isNotValidImportRequest(AtlasAsyncImportRequest importRequest) {
        return importRequest == null ||
                (!AtlasAsyncImportRequest.ImportStatus.WAITING.equals(importRequest.getStatus()) && !AtlasAsyncImportRequest.ImportStatus.PROCESSING.equals(importRequest.getStatus()));
    }
    ...
    private void resumeInProgressImports() {
        ...
        try {
            ...
            LOG.info("Resuming import if resources available: {}", importId);
