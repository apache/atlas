...
const TablePagination: React.FC<PaginationProps> = ({
  ...
  const [value, setValue] = React.useState<any>(
    searchParams.get("pageLimit") !== (undefined || null)
  ...
  let limit =
    searchParams.get("pageLimit") !== (undefined || null)
      ? searchParams.get("pageLimit")
      : pageSize || 0;
  // let pageFrom = isFirstPage ? 1 : Number(pageSize) + 1;
  let pageFrom = isFirstPage ? 1 : pageSize * pageIndex + 1;
  // let pageTo = isFirstPage ? Number(limit) : Number(pageSize) + Number(limit);
        ...
        (!memoizedData.length || memoizedData.length < pagination.pageSize) ? (
          ...
          <>
              ...
              <LightTooltip title="Previous">
                <IconButton
                  ...
                  onClick={() => {
                    ...
                    setPagination((prev) => ({
                      ...prev,
                      pageIndex: prev.pageIndex - 1 // Correctly increment page index
                    }));
                    setRowSelection({});
                    if (isFirstPage) {
                      searchParams.delete("pageOffset");
              ...
              <LightTooltip title="Next">
                <IconButton
                  ...
                  onClick={() => {
                    ...
                    // searchParams.set("pageOffset", Number(pageSize + limit));
                    searchParams.set(
                      "pageOffset",
                      `${pagination.pageSize * (pagination.pageIndex + 1)}`
                    );
            ...
            <IconButton
              ...
              disabled={
                searchParams.get("searchType") == "basic" && !getCanNextPage()
              }
