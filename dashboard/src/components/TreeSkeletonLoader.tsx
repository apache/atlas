...
const TreeSkeletonLoader = ({ count = 7 }: { count?: number }) => {
  const treeItemSkeleton = (indentLevel: number, textWidth: string, key: number) => (
    <Stack key={key} direction="row" alignItems="center" gap="12px" paddingLeft={`${indentLevel * 24}px`}>
      <SkeletonLoader className="tree-skeleton-item" animation="pulse" variant="rectangular" width={16} height={16} count={1} />
      <SkeletonLoader className="tree-skeleton-item" animation="pulse" variant="rectangular" width={textWidth} height={20} count={1} />
    </Stack>
  );
