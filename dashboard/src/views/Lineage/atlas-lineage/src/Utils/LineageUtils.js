...
const LineageUtils = {
    ...
    var dragHelper = {
      ...
      intersectRect: (node, point) => {
        var x = node.x,
          ...
          dy = point.y - y,
          nodeDistance = guid
            ? this.nodeArrowDistance + 3
            : this.nodeArrowDistance,
          w = nodeDistance,
          h = nodeDistance,
          sx = 0,
          sy = 0;
  ...
  zoom: function ({ svg, xa, ya, scale }) {
    svg
      .transition()
      .duration(750)
      .call(this.d3Zoom.transform, zoomIdentity.translate(xa, ya).scale(scale));
  },
  fitToScreen: function ({ svg }) {
    ...
    var scale =
        (scale || 0.95) / Math.max(width / fullWidth, height / fullHeight),
  ...
  centerNode: function ({
    ...
    if (!isSelected) {
      svg.call(
        ...
        zoomIdentity.translate(xa, ya).scale(scale)
      );
    } else {
      svg
        .transition()
  ...
  getToolTipDirection: function ({ el }) {
    ...
    if (width - currentELWidth.left < 330) {
      direction =
        width - currentELWidth.left < 330 && currentELWidth.top < 400
          ? "sw"
          : "w";
      ...
      direction =
        width - currentELWidth.left < 330 && currentELWidth.top > 600
          ? "nw"
          : "n";
  ...
  getEntityIconPath: function ({ entityData, errorUrl, imgBasePath }) {
    ...
    if (entityData) {
      ...
      function getImgPath(imageName) {
        return (
          iconBasePath +
          (Enums.entityStateReadOnly[status]
            ? "disabled/" + imageName
            : imageName)
        );
      }
      ...
      if (errorUrl) {
        ...
        if (errorUrl.indexOf("table.png") > -1) {
          //removed condition for default process image
          return null;
        }
        ...
        if (serviceType && isErrorInTypeName) {
          // var imageName = serviceType.toLowerCase() + ".png";
        ...
        // var imageName = typeName.toLowerCase() + ".png";
  ...
  imgShapeRender: function (
    parent,
    bbox,
    node,
    { dagreD3, defsEl, imgBasePath, guid, isRankdirToBottom }
  ) {
    var that = this,
      viewGuid = guid,
      imageIconPath = node.btnType
        ? "/img/entity-icon/expandBtn.svg"
        : this.getEntityIconPath({ entityData: node, imgBasePath }),
    ...
    var shapeSvg = parent
      ...
      .attr(
        "class",
        "nodeImage " +
          (currentNode ? "currentNode" : node.isProcess ? "process" : "node")
      );
    if (currentNode) {
    ...
    function svgPattern(patternConfig) {
      if (
        defsEl.select('pattern[id="img_' + patternConfig.imgName + '"]').empty()
      ) {
        defsEl
          .append("pattern")
          .attr("x", "0%")
              ...
              var getImageData = function (options) {
                ...
                xhr.onreadystatechange = function () {
                  if (xhr.readyState === 4) {
                    if (
                      xhr.status === 200 &&
                      (xhr.response.type == "image/png" ||
                        xhr.response.type == "image/svg+xml")
                    ) {
                      if (platform.name !== "IE") {
                        ...
                        that.imageObject[patternConfig.imageIconPath] =
                          imagePath;
                      }
                      if (
                        patternConfig.imageIconPath !==
                      ...
                      xhr.status === 404 ||
                      xhr.response.type == "text/html"
                    ) {
                      const imgPath = that.getEntityIconPath({
                        entityData: node,
                        errorUrl: imagePath,
                      });
                      if (imgPath === null) {
    ...
    node.intersect = function (point) {
      return dagreD3.intersect.circle(
        node,
        currentNode ? that.nodeArrowDistance + 3 : that.nodeArrowDistance,
        point
      );
    };
    return shapeSvg;
  ...
  saveSvg: function ({
    ...
    setTimeout(function () {
      ...
      svgClone.attributes.viewBox.value =
        xAxis + "," + yAxis + "," + setWidth + "," + setHeight;
        ...
        try {
          ...
          canvas.toBlob(function (blob) {
            if (!blob) {
              onExportLineage({
                status: "failed",
                message: "There was an error in downloading Lineage!",
              });
              return;
            }
