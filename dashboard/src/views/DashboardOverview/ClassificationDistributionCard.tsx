...
import {
  navigateToSearch,
  navigateToClassificationSearch,
...
const ClassificationDistributionCard = memo(
  ({ tag, isLoading }: ClassificationDistributionCardProps) => {
    const navigate = useNavigate();
    const data = getClassificationDistribution(tag, 5);
    const associationTotal = useMemo(
      () => getTagEntityAssociationTotal(tag),
      [tag],
    );
    ...
    const renderTooltip = useCallback((props: unknown) => {
      ...
      if (!row) return null;
      return (
        <Box className="classification-distribution-card__tooltip">
          <Typography
            variant="body2"
            fontWeight={600}
    ...
    return (
      <Paper elevation={1} className="classification-distribution-card__paper">
        <Box className="classification-distribution-card__header">
          ...
          >
            ...
            <Link
              ...
              className="classification-distribution-card__view-all-link"
        ...
        <Typography
          variant="caption"
          className="classification-distribution-card__description"
        >
          The chart shows the top 5 classifications by number of entities in
          use.
        </Typography>
        {data.length === 0 ? (
          ...
          <Box className="classification-distribution-card__chart-container">
            <ResponsiveContainer
              width="100%"
              height="100%"
              ...
              >
                ...
                <YAxis
                  ...
                  tick={(props: Record<string, unknown>) => {
                    ...
                    const displayLabel =
                      truncateClassificationYAxisLabel(value);
                    const isTruncated =
                      isClassificationYAxisLabelTruncated(value);
                    return (
                      <g
                        transform={`translate(${x},${y})`}
                        onClick={() =>
                          value ? handleLabelClick(value) : undefined
                        }
                        className={`classification-distribution-card__y-axis-tick-${value ? "pointer" : "default"}`}
                ...
                >
                  <LabelList
                    ...
                    className="classification-distribution-card__bar-label"
                  />
                  {data.map((_, index) => (
