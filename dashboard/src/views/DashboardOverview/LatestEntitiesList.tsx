...
const LatestEntitiesList = memo(({ entities, isLoading, error }: LatestEntitiesListProps) => {
	...
	const handleViewAll = useCallback(() => {
		navigateToLatestEntitiesSearch(navigate, Date.now());
	}, [navigate]);
				...
				<List disablePadding sx={{ pt: 2 }}>
					{entities.slice(0, 7).map((entity) => {
						const name =
							...
							entity.guid ||
							"Unknown";
						const typeName = entity.typeName || "Entity";
						...
						return (
							...
							>
								<Stack width="100%" direction="row" justifyContent="space-between" alignItems="center">
									<Stack direction="row" spacing={0.5} flex={1} overflow="hidden">
										<Link
											component={RouterLink}
											to={`/detailPage/${entity.guid}`}
											underline="hover"
											color="primary"
											sx={{
												fontSize: "0.875rem",
