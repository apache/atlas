...
import { lineageDepth } from "@utils/Global";
...
const LineageTab = ({ entity }: any) => {
  ...
  const [checkedDeletedEntity, setCheckedDeletedEntity] = useState(true);
    ...
    try {
      ...
      if (isLineageOnDemandEnabled) {
        ...
        response = await getLineageData(
          guid as string,
          !isEmpty(value) ? { depth: value.label } : {}
        );
      }

      const { data } = response;
      let lineageObj = { ...data };
      lineageObj["legends"] = legends ? legends : true;
    ...
    if (!isEmpty(data)) {
      const LineageHelperRef = new LineageHelper({
        ...
        isShowHoverPath: function () {
          setCurrentPathChecked(true);
          return true;
        },
        isShowTooltip: function () {
          setNodeDetailsChecked(true);
          return true;
        },
  ...
  const handleChange = (newValue: any) => {
    // lineageDepth = parseInt(newValue);
    if (!isLineageOnDemandEnabled) {
      fetchGraph({
  ...
  const onCheckUnwantedEntity = (
    ...
    if (type === "checkHideProcess") {
      filterObj.isProcessHideCheck = e.target.checked;
    } else {
      filterObj.isDeletedEntityHideCheck = e.target.checked;
    }
