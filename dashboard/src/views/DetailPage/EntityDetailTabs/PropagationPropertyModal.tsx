...
const PropagationPropertyModal = ({
  ...
  const handlePropagationChange = (e, newValues) => {
    setPropagationChecked(e.currentTarget.checked);
  };
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ row, value, column: { id }, updateData }) => {
          ...
          const { fromBlockClassification } = values;

          return (
            <input
              type="checkbox"
              checked={propagationChecked}
              onChange={(e) => setPropagationChecked(e.currentTarget.checked)}
            />
          );
        },
