...
const BMAttributes = ({ loading, bmAttributes, entity }: any) => {
  ...
  //   const [fieldsOptions, setFieldOptions] = useState(bmOptions);
  ...
  // let bmAttributesValues: any = watch("businessMetadata");
  ...
  const onSubmit = async (values: { businessMetadata: any }) => {
    ...
    for (let dataObj of businessMetadata) {
      ...
      if (atrributeType.includes(typeName)) {
        ...
        data[group] = {
          ...data[group],
          ...{ [label]: dataObj.value.map((obj: { label: any }) => obj.label) }
        };
