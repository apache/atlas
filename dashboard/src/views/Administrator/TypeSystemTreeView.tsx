...
import { lineageDepth, lineageDepth2 } from "@utils/Global";
...
const TypeSystemTreeView = ({ entity }: any) => {
  ...
  const fetchGraph = (options: any) => {
    let typeDef = !isEmpty(entityData?.entityDefs)
      ? cloneDeep(entityData?.entityDefs)
      : [];
  ...
  const createGraph = (opt) => {
    if (lineageHelperRef.current) {
      lineageHelperRef.current.createGraph(); // Example method from LineageHelper
    }

    // LineageHelperRef.createGraph();
  };
          ...
          <Stack padding="1rem 0.5rem">
            <Stack gap="1rem">
              <Stack direction="row" gap="0.5rem">
                <Typography className="menuitem-label">Search Type:</Typography>
              </Stack>
