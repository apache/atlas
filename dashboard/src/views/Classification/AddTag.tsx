...
const AddTag = (props: {
  ...
  const { guid: entityGuid, classifications }: { guid: string } = entityData;
  ...
  const editFormValues = () => {
    ...
    defaultValue["clasification"] = typeName;
  ...
  const clasification = isAdd ? watch("classification") : entityData.typeName;
  ...
  const classificationObj = !isEmpty(clasification)
    ? classificationDefs.find((obj: { name: string }) => {
        return obj.name == (isAdd ? clasification.label : clasification);
      })
    : null;
  ...
  const onSubmit = async (formData: any) => {
    ...
    let classificationDataObj: any = {
      ...
      typeName: isAdd ? clasification.label : classification.label
    };
