...
const GlossaryTree = ({ sideBarOpen, drawerWidth, searchTerm }: Props) => {
  ...
  >([]);
  const [glossaryLoader, setGlossaryLoader] = useState<boolean>(false);

  useEffect(() => {
    setGlossaryLoader(true);
    dispatch(fetchGlossaryData());
    setGlossaryLoader(false);
  }, []);
