...
import { needToValidateDate } from "./Global";
import {
  convertToValidDate,
  formatedDate,
  getUrlState,
            ...
            if (obj.operator === "TIME_RANGE") {
              if (value.indexOf("-") > -1) {
                url[2] = value
                  .split(" - ")
                  .map(function (udKey) {
                    return needToValidateDate
                      ? Date.parse(convertToValidDate(udKey?.trim())).toString()
              ...
              url[2] = needToValidateDate
                ? Date.parse(convertToValidDate(value))
                : Date.parse(value);
            }
            if (type) {
              if (type == "date") {
