...
public class AtlasEntityRelationshipAttributeTest {
    ...
    // ==================== Tests for removed called AFTER added ====================

    /**
     * Test that when setRemovedRelationshipAttribute is called AFTER setAddedRelationshipAttribute,
     * the same entity is removed from addedRelationshipAttributes.
     */
    @Test
    public void testSetRemovedAfterAddedRemovesDuplicateFromAdded() {
        AtlasEntity entity = new AtlasEntity("TestEntity");
        ...
        // Now, set the same readme as removed (this could happen if order of operations is reversed)
        Map<String, String> removedReadme = new HashMap<>();
        removedReadme.put("guid", README_GUID);
        removedReadme.put("typeName", README_TYPE);
        entity.setRemovedRelationshipAttribute(README_ATTR, removedReadme);
        ...
        // Verify added is now empty (deduplication occurred)
        assertNull(entity.getAddedRelationshipAttributes().get(README_ATTR),
                "Same entity should not appear in both added and removed relationship attributes");
    }

    /**
     * Test that addOrAppendRemovedRelationshipAttribute called AFTER addOrAppendAddedRelationshipAttribute
     * removes matching entries from addedRelationshipAttributes list.
     */
    @Test
    public void testAddOrAppendRemovedAfterAddedRemovesDuplicateFromAddedList() {
        AtlasEntity entity = new AtlasEntity("TestEntity");
        ...
        // Now, append the same readme to removed list
        AtlasObjectId removedReadme = new AtlasObjectId(README_GUID, README_TYPE);
        entity.addOrAppendRemovedRelationshipAttribute(README_ATTR, removedReadme);
        ...
        // Verify added list is now empty or the attribute is removed (deduplication occurred)
        Object addedAfterRemove = entity.getAddedRelationshipAttributes().get(README_ATTR);
        assertTrue(addedAfterRemove == null || (addedAfterRemove instanceof List && ((List<?>) addedAfterRemove).isEmpty()),
                "Same entity should be removed from addedRelationshipAttributes when removed is set after");
    }

    /**
     * Test partial deduplication when removed is called after added - only matching items removed.
     */
    @Test
    public void testPartialDeduplicationWhenRemovedAfterAdded() {
        AtlasEntity entity = new AtlasEntity("TestEntity");
        String otherGuid = "other-guid-789";

        // Add two items to added list
        entity.addOrAppendAddedRelationshipAttribute(README_ATTR, new AtlasObjectId(README_GUID, README_TYPE));
        entity.addOrAppendAddedRelationshipAttribute(README_ATTR, new AtlasObjectId(otherGuid, README_TYPE));

        // Verify added list has 2 items
        List<?> addedList = (List<?>) entity.getAddedRelationshipAttributes().get(README_ATTR);
        assertEquals(addedList.size(), 2);

        // Remove one of them (should only remove matching one from added)
        entity.addOrAppendRemovedRelationshipAttribute(README_ATTR, new AtlasObjectId(README_GUID, README_TYPE));

        // Verify added list now has only 1 item (the non-matching one)
        addedList = (List<?>) entity.getAddedRelationshipAttributes().get(README_ATTR);
        assertEquals(addedList.size(), 1, "Only matching guid should be removed from added list");
    }
}
