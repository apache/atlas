...
public abstract class BasicTestSetup extends AtlasTestBase {
    ...
    public AtlasEntity.AtlasEntitiesWithExtInfo hiveTestEntities() {
        ...
        AtlasEntity sd = storageDescriptor("hdfs://host:8000/apps/warehouse/sales", "TextInputFormat", "TextOutputFormat", true, ImmutableList.of(column("time_id", "int", "time id")));

        entities.add(sd);

        List<AtlasEntity> salesFactColumns = ImmutableList.of(column("time_id", "int", "time id"),
        ...
        List<AtlasEntity> logFactColumns = ImmutableList.of(column("time_id", "int", "time id"),
        ...
        List<AtlasEntity> timeDimColumns = ImmutableList.of(column("time_id", "int", "time id"),
        ...
        timeDim = table("time_dim", "time dimension table", salesDB, sd, "John Doe", "External", timeDimColumns, DIMENSION_CLASSIFICATION);

        entities.add(timeDim);

        AtlasEntity reportingDB = database("Reporting", "/apps/warehouse/reporting database", "Jane BI", "hdfs://host:8000/apps/warehouse/reporting");
        ...
        AtlasEntity salesFactDaily = table("sales_fact_daily_mv", "sales fact daily materialized view", reportingDB, sd, "Joe BI", "Managed", salesFactColumns, METRIC_CLASSIFICATION);
        ...
        AtlasEntity loggingFactDaily = table("log_fact_daily_mv", "log fact daily materialized view", logDB, sd, "Tim ETL", "Managed", logFactColumns, LOGDATA_CLASSIFICATION);

        entities.add(loggingFactDaily);

        List<AtlasEntity> productDimColumns = ImmutableList.of(column("product_id", "int", "product id"),
        ...
        productDim = table("product_dim", "product dimension table", salesDB, sd, "John Doe 2", "Managed", productDimColumns, DIMENSION_CLASSIFICATION);
        ...
        List<AtlasEntity> customerDimColumns = ImmutableList.of(column("customer_id", "int", "customer id", PII_CLASSIFICATION),
        ...
        AtlasEntity customerDim = table("customer_dim", "customer dimension table", salesDB, sd, "fetl", "External", customerDimColumns, DIMENSION_CLASSIFICATION);
        ...
        AtlasEntity salesFactMonthly = table("sales_fact_monthly_mv", "sales fact monthly materialized view", reportingDB, sd, "Jane BI", "Managed", salesFactColumns, METRIC_CLASSIFICATION);
        ...
        loggingFactMonthly = table("logging_fact_monthly_mv", "logging fact monthly materialized view", logDB, sd, "Tim ETL 2", "Managed", logFactColumns, LOGDATA_CLASSIFICATION);
    ...
    public EntityMutationResponse createDummyEntity(String name, String type, String... traitNames) throws AtlasBaseException {
        ...
        return entityStore.createOrUpdate(new AtlasEntityStream(new AtlasEntity.AtlasEntitiesWithExtInfo(entity)), false);
    }
    ...
    protected AtlasEntity table(String name, String description, AtlasEntity db, AtlasEntity sd, String owner, String tableType,
        ...
        if (name.equals("time_dim")) {
            AtlasClassification classification = new AtlasClassification(traitNames[0], Collections.singletonMap("timeAttr", "timeValue"));

            table.setClassifications(Collections.singletonList(classification));
        } else if (name.equals("product_dim")) {
            AtlasClassification classification = new AtlasClassification(traitNames[0], Collections.singletonMap("productAttr", "productValue"));
