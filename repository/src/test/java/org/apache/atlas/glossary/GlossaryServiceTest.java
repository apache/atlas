...
public class GlossaryServiceTest {
    ...
    public void testUpdateGlossary() {
        try {
            ...
            AtlasGlossary glossary = glossaryService.getGlossary(bankGlossary.getGuid());
            glossary.setShortDescription("Updated short description");
            glossary.setLongDescription("Updated long description");

            AtlasGlossary updatedGlossary = glossaryService.updateGlossary(glossary);
            assertNotNull(updatedGlossary);
            assertEquals(updatedGlossary.getGuid(), bankGlossary.getGuid());
            assertEquals(updatedGlossary, glossary);
        } catch (AtlasBaseException e) {
    ...
    public void testUpdateGlossaryTerm() {
        for (AtlasGlossaryTerm t : Arrays.asList(checkingAccount, savingsAccount, fixedRateMortgage, adjustableRateMortgage)) {
            try {
                AtlasGlossaryTerm glossaryTerm = glossaryService.getTerm(t.getGuid());
                glossaryTerm.setShortDescription("Updated short description");
                glossaryTerm.setLongDescription("Updated long description");

                AtlasGlossaryTerm updatedTerm = glossaryService.updateTerm(glossaryTerm);
    ...
    public void testAddTermsToCategory() {
        AtlasGlossaryCategory updatedAccCategory = accountCategory;
        for (AtlasGlossaryTerm termEntry : Arrays.asList(checkingAccount, savingsAccount)) {
            ...
            updatedAccCategory.addTerm(relatedTermId);
        }
        try {
            AtlasGlossaryCategory updated = glossaryService.updateCategory(updatedAccCategory);
        ...
        AtlasGlossaryCategory updatedMtgCategory = mortgageCategory;
        for (AtlasGlossaryTerm termEntry : Arrays.asList(fixedRateMortgage, adjustableRateMortgage)) {
            AtlasRelatedTermHeader relatedTermId = new AtlasRelatedTermHeader();
            relatedTermId.setTermGuid(termEntry.getGuid());
            relatedTermId.setStatus(AtlasTermRelationshipStatus.ACTIVE);
            relatedTermId.setSteward("UT");
            ...
            updatedMtgCategory.addTerm(relatedTermId);
        }
        try {
            AtlasGlossaryCategory updated = glossaryService.updateCategory(updatedMtgCategory);
            assertNotNull(updated.getTerms());
            assertEquals(updated.getTerms().size(), 2);
        } catch (AtlasBaseException e) {
            fail("Glossary category update should've succeeded", e);
    ...
    public void testAddGlossaryCategoryChildren() {
        ...
        try {
            AtlasGlossaryCategory updateGlossaryCategory = glossaryService.updateCategory(parent);
    ...
    public void testCategoryRelation() {
        ...
        Map<String, List<AtlasGlossaryCategory>> relatedCategories;
        try {
    ...
    public void testGetGlossaryCategories(int offset, int limit, int expected) {
        ...
        try {
            List<AtlasGlossaryCategory> glossaryCategories = glossaryService.getGlossaryCategories(guid, offset, limit, sortOrder);
    ...
    public void testGetCategoryTerms(int offset, int limit, int expected) {
        for (AtlasGlossaryCategory c : Arrays.asList(accountCategory, mortgageCategory)) {
            try {
                List<AtlasGlossaryTerm> categoryTerms = glossaryService.getCategoryTerms(c.getGuid(), offset, limit, SortOrder.ASCENDING);
    ...
    public void testGetRelatedTerms() {
        for (AtlasGlossaryTerm term : Arrays.asList(checkingAccount, savingsAccount, fixedRateMortgage, adjustableRateMortgage)) {
            try {
                Map<String, List<AtlasGlossaryTerm>> relatedTerms = glossaryService.getRelatedTerms(term.getGuid(), 0, 5, SortOrder.ASCENDING);
                assertNotNull(relatedTerms);
                assertFalse(relatedTerms.isEmpty());
                List<AtlasGlossaryTerm> terms = relatedTerms.get(AtlasGlossaryTerm.Relation.SEE_ALSO.getRelationAttrName());
                assertNotNull(terms);
                assertEquals(terms.size(), 1);
            } catch (AtlasBaseException e) {
                fail("Related terms retrieval should've been a success", e);
            }
        }
    }
}