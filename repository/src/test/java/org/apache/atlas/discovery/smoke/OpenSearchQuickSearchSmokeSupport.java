...
final class OpenSearchQuickSearchSmokeSupport {
    ...
    static void bootstrapApplicationProperties() throws Exception {
        // Load from the test classpath (same pattern as graphdb/janus OpenSearch ITs). Do not set atlas.conf to a
        // relative filesystem path — CI runs mvn from the repo root and user.dir may not be this module directory.
    ...
    static void debugOpenSearchSearch(String queryClauseJson) throws IOException {
        String requestBody = toSearchRequestBody(queryClauseJson);
        System.out.println("[DEBUG] OpenSearch POST /" + PHYSICAL_INDEX + "/_search");
        System.out.println("[DEBUG] request body: " + requestBody);
        String response = httpPost("/" + PHYSICAL_INDEX + "/_search", requestBody);
        System.out.println("[DEBUG] response (truncated): "
                + (response.length() > 500 ? response.substring(0, 500) + "..." : response));
    }

    /** Wraps a Query DSL clause in the top-level {@code query} object required by {@code _search}. */
    static String toSearchRequestBody(String queryClauseJson) {
        String trimmed = queryClauseJson.trim();
    ...
    static String httpPost(String path, String jsonBody) throws IOException {
        ...
        java.io.InputStream in = status >= 400 ? connection.getErrorStream() : connection.getInputStream();
        String response = in != null ? new String(in.readAllBytes(), StandardCharsets.UTF_8) : "";
        if (status != 200) {
            throw new IOException("POST " + path + " returned HTTP " + status
