...
/**
 * Comprehensive test class for SearchResultDownloadTask targeting 90%+ coverage
 */
public class SearchResultDownloadTaskTest {
    ...
    public void setUp() {
        ...
        // Note: AtlasConfiguration.SEARCH_MAX_LIMIT uses default value in tests
    }
    ...
    public void testDownloadDirPathInitialization() throws Exception {
        // Test that DOWNLOAD_DIR_PATH is properly initialized
    ...
    private void setupMocksForBasicSearch() throws AtlasBaseException {
        SearchParameters searchParams = new SearchParameters();
        atlasJsonMock.when(() -> AtlasJson.fromJson(anyString(), eq(SearchParameters.class))).thenReturn(searchParams);
    ...
    private AtlasSearchResult createMockSearchResultWithComplexAttributes() {
        AtlasSearchResult searchResult = new AtlasSearchResult();
        ...
        entity1.setGuid("guid1");
        entity1.setDisplayText("TestTable1");
        entity1.setClassificationNames(Arrays.asList("PII"));
        entity1.setMeaningNames(Arrays.asList("BusinessTerm1"));

        Map<String, Object> attributes = new HashMap<>();

        // Add AtlasObjectId attribute
        AtlasObjectId objectId = new AtlasObjectId();
        objectId.setGuid("ref-guid");
        Map<String, Object> uniqueAttrs = new HashMap<>();
        uniqueAttrs.put("qualifiedName", "db.table@cluster");
        objectId.setUniqueAttributes(uniqueAttrs);
        attributes.put("database", objectId);

        // Add List attribute
        List<String> listAttr = Arrays.asList("val1", "val2");
        attributes.put("columns", listAttr);

        // Add List of AtlasObjectId
        AtlasObjectId objId1 = new AtlasObjectId();
        objId1.setGuid("col-guid1");
        Map<String, Object> uniqueAttrs1 = new HashMap<>();
        uniqueAttrs1.put("qualifiedName", "db.table.col1@cluster");
        objId1.setUniqueAttributes(uniqueAttrs1);

        List<AtlasObjectId> objList = Arrays.asList(objId1);
        attributes.put("references", objList);
    ...
    private AtlasSearchResult createMockSearchResultWithAttributeResult() {
        AtlasSearchResult searchResult = new AtlasSearchResult();
        searchResult.setEntities(new ArrayList<>());

        AtlasSearchResult.AttributeSearchResult attributeResult = new AtlasSearchResult.AttributeSearchResult();
        attributeResult.setName(Arrays.asList("name", "owner"));
        attributeResult.setValues(Arrays.asList(Arrays.asList("Table1", "user1"), Arrays.asList("Table2", "user2")));
        searchResult.setAttributes(attributeResult);
    ...
    private void invokeGenerateCSVMethod(AtlasSearchResult searchResult, Map<String, String> attributeLabelMap,
                                        AtlasSearchResult.AtlasQueryType queryType) throws Exception {
        Method generateCSVMethod = SearchResultDownloadTask.class.getDeclaredMethod("generateCSVFileFromSearchResult", AtlasSearchResult.class, Map.class, AtlasSearchResult.AtlasQueryType.class);
        generateCSVMethod.setAccessible(true);
        generateCSVMethod.invoke(downloadTask, searchResult, attributeLabelMap, queryType);
    }
