...
/**
 * Unit test class for AtlasEnumFormatConverter.
 * Tests enum format conversion functionality including validation and V1/V2 conversions.
 */
public class AtlasEnumFormatConverterTest {
    ...
    public void testFromV1ToV2WithMapEmptyValue() throws AtlasBaseException {
        ...
        // When - When there's no "value" or "ordinal" key, it treats the whole object as the value
        // This test expects the behavior that an empty map (without value/ordinal) returns null
        Object result = converter.fromV1ToV2(map, mockEnumType, mockContext);
        // Then
        // Since the converter processes the whole object as v1Value if no "value" or "ordinal" is found,
        // it will try to convert the map object itself. The test should expect "TEST_VALUE" as configured in setup
        assertEquals(result, "TEST_VALUE");
    }
