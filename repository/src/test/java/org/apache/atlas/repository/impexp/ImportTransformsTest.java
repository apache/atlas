...
public class ImportTransformsTest {
    ...
    private static final String COLUMN_QUALIFIED_NAME_FORMAT    = "col%s.TABLE1.default@cl1";
    private static final String HIVE_TABLE_ATTR_SYNC_INFO       = "hive_table.syncInfo";
    private static final String HIVE_TABLE_ATTR_REPLICATED_FROM = "replicatedFromCluster";
    private static final String HIVE_TABLE_ATTR_REPLICATED_TO   = "replicatedToCluster";
    ...
    public void removeClassificationTransform_RemovesSpecifiedClassification() throws AtlasBaseException {
        ...
        AtlasEntity entity                = getHiveTableAtlasEntity();
        String      expectedQualifiedName = entity.getAttribute(ATTR_NAME_QUALIFIED_NAME).toString().replace("@cl1", "@cl2");
        ...
        assertNotNull(t);
        assertEquals(entity.getAttribute(ATTR_NAME_QUALIFIED_NAME), expectedQualifiedName);
    }

    @Test
    public void add_setsValueOfAttribute() throws AtlasBaseException {
        final String     expectedSyncInfo = "cl1:import";
        String           s                = String.format(jsonReplaceAndAddAttrValue, "cl1", "cl2", HIVE_TABLE_ATTR_SYNC_INFO, expectedSyncInfo);
        ImportTransforms t                = ImportTransforms.fromJson(s);
        ...
        assertNotNull(t);
        assertEquals(entity.getAttribute(ATTR_NAME_QUALIFIED_NAME), expectedQualifiedName);
        assertEquals(entity.getAttribute(HIVE_TABLE_ATTR_SYNC_INFO), new ArrayList<>(Collections.singletonList(expectedSyncInfo)));
    }
    ...
    public void clearAttrValue_removesValueOfAttribute() throws AtlasBaseException {
        ...
        assertFalse(t.getTransforms().isEmpty());
