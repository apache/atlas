...
//        atlasConfigurationMock.when(() -> AtlasConfiguration.REBUILD_INDEX.getBoolean()).thenReturn(false);

        patch.apply();

        // When rebuild index is disabled, the patch should return early without setting status
        verify(management, never()).updateUniqueIndexesForConsistencyLock();
    }

/*    @Test
    public void testApplyWhenRebuildIndexEnabled() throws AtlasBaseException {
//        atlasConfigurationMock.when(() -> AtlasConfiguration.REBUILD_INDEX.getBoolean()).thenReturn(true);
...
//        atlasConfigurationMock.when(() -> AtlasConfiguration.REBUILD_INDEX.getBoolean()).thenReturn(true);
        ...
        // The apply method catches all exceptions, so it should not throw
        patch.apply();
    ...
    public void testReindexPatchProcessorRepairElementsMethod() throws Exception {
        ...
        // Create a mock BiConsumer with correct signature
    ...
    public void testReindexPatchProcessorWithManagementException() throws Exception {
        ...
        // The repair methods should handle exceptions gracefully
        Method repairVerticesMethod = ReIndexPatch.ReindexPatchProcessor.class.getDeclaredMethod("repairVertices");
        repairVerticesMethod.setAccessible(true);
        repairVerticesMethod.invoke(processor);
