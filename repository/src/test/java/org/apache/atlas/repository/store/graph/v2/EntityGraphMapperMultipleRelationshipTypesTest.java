...
public class EntityGraphMapperMultipleRelationshipTypesTest extends AtlasEntityTestBase {
    ...
    /** Same mixed-type {@code tables} payload as a list and as a set; two batches so each hive_db GUID is easy to resolve. */
    @Test
    public void testMixedRelationshipTypesWithListAndSetCollection() throws Exception {
        init();

        AtlasEntity dbList   = createHiveDbEntity("list_db_" + randomString());
        AtlasEntity hiveTbl  = createHiveTableEntity("mix_ht_" + randomString());
        AtlasEntity deltaTbl = createDeltaTableEntity("mix_dt_" + randomString());

        AtlasEntitiesWithExtInfo listBatch = new AtlasEntitiesWithExtInfo();
        listBatch.addEntity(dbList);
        listBatch.addEntity(hiveTbl);
        listBatch.addEntity(deltaTbl);

        EntityMutationResponse listResp = entityStore.createOrUpdate(new AtlasEntityStream(listBatch), false);
        assertNotNull(listResp);

        String dbListGuid = findEntityGuid(listResp.getCreatedEntities(), HIVE_DB_TYPE);
        String htGuid     = findEntityGuid(listResp.getCreatedEntities(), HIVE_TABLE_TYPE);
        String dtGuid     = findEntityGuid(listResp.getCreatedEntities(), DELTA_TABLE_TYPE);

        AtlasRelatedObjectId hiveRef  = new AtlasRelatedObjectId(new AtlasObjectId(htGuid, HIVE_TABLE_TYPE), HIVE_TABLE_DB_RELATIONSHIP);
        AtlasRelatedObjectId deltaRef = new AtlasRelatedObjectId(new AtlasObjectId(dtGuid, DELTA_TABLE_TYPE), DELTA_TABLE_DB_RELATIONSHIP);

        List<AtlasRelatedObjectId> asList = new ArrayList<>();
        asList.add(hiveRef);
        asList.add(deltaRef);

        AtlasEntity updateList = new AtlasEntity(HIVE_DB_TYPE);
        updateList.setGuid(dbListGuid);
        updateList.setAttribute("name", dbList.getAttribute("name"));
        updateList.setRelationshipAttribute(TABLES_ATTR_NAME, asList);
        assertNotNull(entityStore.createOrUpdate(new AtlasEntityStream(new AtlasEntityWithExtInfo(updateList)), false));
        verifyRelationships(dbListGuid, htGuid, dtGuid);

        AtlasEntity dbSet    = createHiveDbEntity("set_db_" + randomString());
        AtlasEntity hiveTbl2 = createHiveTableEntity("mix_ht2_" + randomString());
        AtlasEntity deltaTbl2 = createDeltaTableEntity("mix_dt2_" + randomString());

        AtlasEntitiesWithExtInfo setBatch = new AtlasEntitiesWithExtInfo();
        setBatch.addEntity(dbSet);
        setBatch.addEntity(hiveTbl2);
        setBatch.addEntity(deltaTbl2);

        EntityMutationResponse setResp = entityStore.createOrUpdate(new AtlasEntityStream(setBatch), false);
        assertNotNull(setResp);

        String dbSetGuid = findEntityGuid(setResp.getCreatedEntities(), HIVE_DB_TYPE);
        String ht2Guid   = findEntityGuid(setResp.getCreatedEntities(), HIVE_TABLE_TYPE);
        String dt2Guid   = findEntityGuid(setResp.getCreatedEntities(), DELTA_TABLE_TYPE);

        Set<AtlasRelatedObjectId> asSet = new HashSet<>();
        asSet.add(new AtlasRelatedObjectId(new AtlasObjectId(ht2Guid, HIVE_TABLE_TYPE), HIVE_TABLE_DB_RELATIONSHIP));
        asSet.add(new AtlasRelatedObjectId(new AtlasObjectId(dt2Guid, DELTA_TABLE_TYPE), DELTA_TABLE_DB_RELATIONSHIP));

        AtlasEntity updateSet = new AtlasEntity(HIVE_DB_TYPE);
        updateSet.setGuid(dbSetGuid);
        updateSet.setAttribute("name", dbSet.getAttribute("name"));
        updateSet.setRelationshipAttribute(TABLES_ATTR_NAME, asSet);
        assertNotNull(entityStore.createOrUpdate(new AtlasEntityStream(new AtlasEntityWithExtInfo(updateSet)), false));
        verifyRelationships(dbSetGuid, ht2Guid, dt2Guid);
    }
