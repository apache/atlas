...
public class GlossaryService {
    ...
    public List<AtlasGlossaryCategory> getGlossaryCategories(String glossaryGuid, int offset, int limit, SortOrder sortOrder) throws AtlasBaseException {
        ...
        if (CollectionUtils.isNotEmpty(glossary.getCategories())) {
            List<AtlasGlossaryCategory> toLoad = glossary.getCategories().stream()
                                                         .map(t -> getAtlasGlossaryCategorySkeleton(t.getCategoryGuid()))
                                                         .collect(Collectors.toList());
            Iterable<AtlasGlossaryCategory> categories = dataAccess.load(toLoad);
            toLoad.clear();
            categories.forEach(toLoad::add);
            ret = new PaginationHelper<>(toLoad, offset, limit).getPaginatedList();
        } else {
            ret = Collections.emptyList();
        }
    ...
    public List<AtlasGlossaryTerm> getCategoryTerms(String categoryGuid, int offset, int limit, SortOrder sortOrder) throws AtlasBaseException {
        if (Objects.isNull(categoryGuid)) {
            throw new AtlasBaseException(AtlasErrorCode.BAD_REQUEST, "categoryGuid is null/empty");
        }
        ...
        if (CollectionUtils.isNotEmpty(glossaryCategory.getTerms())) {
            ret = loadTerms(glossaryCategory.getTerms(), offset, limit, sortOrder);
        } else {
            ret = Collections.emptyList();
        }
    ...
    public Map<String, List<AtlasGlossaryTerm>> getRelatedTerms(String termGuid, int offset, int limit, SortOrder sortOrder) throws AtlasBaseException {
        if (Objects.isNull(termGuid)) {
            throw new AtlasBaseException(AtlasErrorCode.BAD_REQUEST, "termGuid is null/empty");
        }
        ...
        Map<String, List<AtlasGlossaryTerm>> ret;
        if (glossaryTerm.hasTerms()) {
            ret = loadTerms(glossaryTerm.getRelatedTerms(), offset, limit, sortOrder);
        } else {
    ...
    public Map<String, List<AtlasGlossaryCategory>> getRelatedCategories(String categoryGuid, int offset, int limit, SortOrder sortOrder) throws AtlasBaseException {
        if (Objects.isNull(categoryGuid)) {
            throw new AtlasBaseException(AtlasErrorCode.BAD_REQUEST, "categoryGuid is null/empty");
        }
        ...
        Map<String, List<AtlasGlossaryCategory>> ret = new HashMap<>();
        if (glossaryCategory.getParentCategory() != null) {
            AtlasGlossaryCategory parent = getCategory(glossaryCategory.getParentCategory().getCategoryGuid());
            ret.put("parent", new ArrayList<AtlasGlossaryCategory>() {{
                add(parent);
            }});
        }
        if (CollectionUtils.isNotEmpty(glossaryCategory.getChildrenCategories())) {
            List<AtlasGlossaryCategory> children = loadCategories(glossaryCategory.getChildrenCategories(), offset, limit, sortOrder);
