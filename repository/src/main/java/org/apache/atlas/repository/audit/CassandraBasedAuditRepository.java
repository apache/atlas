...
public class CassandraBasedAuditRepository extends AbstractStorageBasedAuditRepository {
    ...
    public void putEventsV1(List<EntityAuditEvent> events) {
        ...
        events.forEach(event -> batch.add(stmt.bind(event.getEntityId(), event.getTimestamp(),
                ...
                (PERSIST_ENTITY_DEFINITION ? event.getEntityDefinitionString() : null))));
    ...
    void initializeSettings() {
        keyspace          = applicationProperties.getString(CASSANDRA_AUDIT_KEYSPACE_PROPERTY, DEFAULT_KEYSPACE);
        replicationFactor = applicationProperties.getInt(CASSANDRA_REPLICATION_FACTOR_PROPERTY, DEFAULT_REPLICATION_FACTOR);
        clusterName       = applicationProperties.getString(CASSANDRA_CLUSTERNAME_PROPERTY, DEFAULT_CLUSTER_NAME);
        port              = applicationProperties.getInt(CASSANDRA_PORT_PROPERTY, DEFAULT_PORT);
        username          = applicationProperties.getString(CASSANDRA_USERNAME_PROPERTY, "");
        password          = applicationProperties.getString(CASSANDRA_PASSWORD_PROPERTY, "");
    }
    ...
    void createSession() throws AtlasException {
        ...
        try {
            ...
            if (cluster.getMetadata().getKeyspace(keyspace) == null) {
                String query = "CREATE KEYSPACE " + keyspace + " WITH replication = {'class':'SimpleStrategy', 'replication_factor':" + replicationFactor + "}; ";
