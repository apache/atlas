                ...
                if (ArrayUtils.isNotEmpty(importFiles)) {
                    ...
                    for (File importFile : importFiles) {
                        if (isValidImportFile(importFile)) {
                            LOG.info("{} with absolute path - {}", importFile.getName(), importFile.getAbsolutePath());

                            ret.add(importFile.getAbsolutePath());
                        } else {
                            LOG.warn("Ignoring {} as it is not a file or does not end with extension {}", FILE_EXTENSION_ZIP, importFile.getAbsolutePath());
                        }
                    }
                } else {
    ...
    private void moveZipFileToArchiveDir(String srcFilePath) {
        ...
        if (!sourceFile.canWrite()) {
            ...
            if (sourceFile.renameTo(new File(newFile))) {
                ...
                LOG.info("Migration Import: Successfully archived the zip file: {} to {}", srcFilePath, this.archiveDir.getAbsolutePath());
            } else {
