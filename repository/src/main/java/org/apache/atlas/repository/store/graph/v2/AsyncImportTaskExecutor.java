...
public class AsyncImportTaskExecutor {
    ...
    public AtlasAsyncImportRequest run(AtlasImportResult result, EntityImportStream entityImportStream) throws AtlasBaseException {
        try {
            ...
            if (ObjectUtils.equals(importRequest.getStatus(), AtlasAsyncImportRequest.ImportStatus.WAITING) ||
                    ObjectUtils.equals(importRequest.getStatus(), AtlasAsyncImportRequest.ImportStatus.PROCESSING)) {
                LOG.warn("Duplicate import request received");
                return importRequest;
            }
            // skip to the most recent published position
            if (ObjectUtils.equals(importRequest.getStatus(), AtlasAsyncImportRequest.ImportStatus.STAGING)) {
    ...
    void publishEntityNotification(AtlasAsyncImportRequest importRequest, EntityImportStream entityImportStream) {
        ...
        while (entityImportStream.hasNext()) {
            ...
            try {
                ...
                LOG.warn("Failed to publish entity: {}", entity.getGuid(), abe);
    ...
    AtlasAsyncImportRequest registerRequest(AtlasImportResult result, String importId, int totalEntities, List<String> creationOrder) throws AtlasBaseException {
        ...
        try {
            ...
            if (existingImportRequest == null
                    || ObjectUtils.equals(existingImportRequest.getStatus(), AtlasAsyncImportRequest.ImportStatus.SUCCESSFUL)
                    || ObjectUtils.equals(existingImportRequest.getStatus(), AtlasAsyncImportRequest.ImportStatus.PARTIAL_SUCCESS)
                    || ObjectUtils.equals(existingImportRequest.getStatus(), AtlasAsyncImportRequest.ImportStatus.FAILED)
                    || ObjectUtils.equals(existingImportRequest.getStatus(), AtlasAsyncImportRequest.ImportStatus.ABORTED)) {
                ...
                LOG.info("atlasAsyncImportRequest : {}, registered as a new request", newImportRequest);
                return newImportRequest;
            }
            // if we are resuming staging, we need to update the latest request received at
            if (ObjectUtils.equals(existingImportRequest.getStatus(), AtlasAsyncImportRequest.ImportStatus.STAGING)) {
            ...
            LOG.info("atlasAsyncImportRequest: {}, is not new hence, resuming", existingImportRequest);
            return existingImportRequest;
        } catch (AtlasBaseException abe) {
            LOG.error("Failed to register import request", abe);
    ...
    public void abortImport(String importId) throws AtlasBaseException {
