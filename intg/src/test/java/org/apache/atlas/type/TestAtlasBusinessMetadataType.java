...
public class TestAtlasBusinessMetadataType {
    ...
    @Test
    public void testResolveReferencesWithStructTypeAttribute() throws AtlasBaseException {
        // Create attribute with struct type (should fail)
        AtlasAttributeDef structAttr = new AtlasAttributeDef("structAttr", "TestStruct", true,
                AtlasAttributeDef.Cardinality.SINGLE, 0, 1, false, false, false,
                "", Collections.emptyList(), Collections.emptyMap(), "Struct attribute", 0, null);

        AtlasBusinessMetadataDef invalidDef = new AtlasBusinessMetadataDef("InvalidBusinessMetadata",
                "Invalid business metadata", "1.0");
        invalidDef.addAttribute(structAttr);

        AtlasBusinessMetadataType invalidType = new AtlasBusinessMetadataType(invalidDef);

        // Mock struct type
        AtlasStructType mockStructType = new AtlasStructType(new AtlasStructDef("TestStruct", "Test struct", "1.0"));

        // This would require more complex mocking, so we'll test the principle
        // The actual implementation checks if attrType instanceof AtlasStructType
    }
