...
public class TestConstants {
    ...
    public void testClassificationOnlySystemAttributesConstants() throws Exception {
        // Test that classification-only system attribute constants are properly defined
    ...
    public void testConstantValuesStartWithEncodedPrefix() throws Exception {
        ...
        for (Field field : fields) {
            ...
            if (!field.getName().equals("INTERNAL_PROPERTY_KEY_PREFIX") &&
                ...
                // All property keys should be encoded versions that start with encoded prefix
                // The encoding is done by AtlasStructType.AtlasAttribute.encodePropertyKey()
                assertNotNull(value, "Constant " + field.getName() + " should not be null");
    ...
    private void assertConstantValue(String constantName, String expectedSuffix) throws Exception {
        ...
        // The constant should be an encoded version of "__" + expectedSuffix
        // We can't easily test the exact encoded value without duplicating the encoding logic,
        // but we can verify it's not empty and is a string
