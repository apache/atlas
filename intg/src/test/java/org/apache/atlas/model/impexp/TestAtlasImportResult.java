...
public class TestAtlasImportResult {
    ...
    public void testSpecialCharactersInStrings() {
        String specialUserName = "user-с-кириллицей-字符-!@#$%^&*()";
        String specialHostName = "host-with-special-chars-äöü-🌟";
        String specialClientIp = "2001:0db8:85a3:0000:0000:8a2e:0370:7334";
    ...
    public void testLargeCollections() {
        // Test with large list of processed entities
    ...
    public void testProcessedEntitiesWithSpecialCharacters() {
        ...
        entities.add("entity-с-кириллицей");
        entities.add("entity-字符");

        importResult.setProcessedEntities(entities);

        assertEquals(importResult.getProcessedEntities().size(), 8);
        assertTrue(importResult.getProcessedEntities().contains("entity-с-кириллицей"));
        assertTrue(importResult.getProcessedEntities().contains("entity-字符"));
    }
    ...
    public void testMetricsWithSpecialKeys() {
        ...
        metrics.put("кириллица", 7);
        metrics.put("中文", 8);

        importResult.setMetrics(metrics);

        assertEquals(importResult.getMetrics().size(), 8);
        assertEquals(importResult.getMetrics().get("кириллица"), Integer.valueOf(7));
        assertEquals(importResult.getMetrics().get("中文"), Integer.valueOf(8));
    }
